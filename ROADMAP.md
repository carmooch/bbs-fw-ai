...
**Phase 2 — Ride feel quick wins.** #3 (per-level ramp rate), #5 (throttle upper deadband), ~~#21 (wheel size dropdown in the tool)~~ done. Each is small. Each is visible. Each requires the config tool to be updated, so this phase forces that work too. #3 and #5 bump `CONFIG_VERSION` to 7 — now safe to do, since the web tool has full config-editing parity.
