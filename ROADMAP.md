...
**Phase 1.5 — Web config tool foundation (added, not in the original plan).** ~~Done~~ — `src/web-tool`, a Web Serial-based browser tool covering the System view's fields, with the protocol layer (checksums, config parse/serialize/validate, the connect/read/write/reset state machine) unit tested without needing a browser or real hardware. CI builds and tests it on every push. Still no real controller to verify the end-to-end Web Serial connection against — that's true of everything in this fork so far, not specific to this phase. The existing config tool (`src/tool`) is a Windows-only WPF/.NET Framework desktop app. The connection to the controller is just a plain serial port at 1200 baud (`System.IO.Ports.SerialPort` in `BbsfwConnection.cs`) over a USB-serial adapter — exactly what the browser's Web Serial API is for. Moving to a browser-based tool now, before Phase 2 sinks more work into WPF-specific UI, avoids redoing that work later. Scope: a Web Serial connection plus the byte protocol layer (read/write config, checksums, opcodes), and just the System view fields that exist today, not a big-bang rewrite of every assist-level editor, wizard, and the event log up front — those get built directly in the web tool as later phases need them. The web tool only needs to understand this fork's current config format (`CONFIG_VERSION` 6+), not the legacy V1-V5 layouts upstream bbs-fw releases used — a rider on a stock install would use the original WPF tool first. The WPF tool stays in the repo as a fallback, frozen rather than actively developed, until the web tool reaches parity and it can be retired. Web Serial is Chromium-only (Chrome/Edge/Opera/Brave) — no Firefox or Safari — which is a real access tradeoff against dropping the Windows-only install barrier.

**Phase 1.6 — Web-tool config-editing parity (added, forced by the architecture).** Bumping `CONFIG_VERSION` for any new field locks the frozen WPF tool out of the new firmware entirely (it rejects any config version above what it knows, on both read and write). Since the WPF tool was the only one that could edit assist levels, the web tool had to gain an assist-level editor *before* the next version bump — otherwise Phase 2 firmware would leave assist levels un-tunable by any tool. Done: ~~#21 (wheel-size preset dropdown, tool-only, no bump)~~ then ~~the web-tool assist-level editor~~ (master-detail Standard/Sport editor + the operation-mode-toggle and startup-level fields). The assist `flags`-byte logic is a faithful, separately-tested port of the WPF view-model; the DOM editor is covered by jsdom tests since this environment can't browser-preview the project. This achieves parity for *config editing* specifically — every editable field, assist levels included, is now in the web tool — which is what makes the version bump below safe. Voltage calibration and the event log remain WPF-only (deferred), so WPF stays as a fallback for those, not fully retired.

**Phase 2 — Ride feel quick wins.** ~~#21 (wheel size dropdown)~~ and ~~#5 (throttle upper deadband, `CONFIG_VERSION` → 7)~~ done. #3 (per-level current ramp rate) is **deliberately split out** rather than bundled: unlike #5 (a safe global byte), #3 needs a one-way change to the `assist_level_t` wire format (6→7 bytes per level) and touches motor current-ramping — the most behaviour-sensitive, hardware-untestable change so far. Isolating it keeps it bisectable and lower-blast-radius. Each feature required the config tool updated, which (since the version bump locks WPF out for config) is now done in *both* the web tool and the WPF tool — WPF stays current because it's still the only tool with voltage calibration and the event log. #5's V7 bump was safe precisely because the web tool reached config-editing parity in Phase 1.6 first.
